	...
	<script>
		function notificar($tipo_actualizacion){
			if ($tipo_actualizacion == 'cover'){
				UIkit.notification({message:'<i class="fa fa-check"></i> La portada del curso ha sido subida con éxito.', status: 'success'});
			}else if ($tipo_actualizacion == 'miniature_cover'){
				UIkit.notification({message:'<i class="fa fa-check"></i> La portada en miniatura del curso ha sido subida con éxito.', status: 'success'});
			}else if ($tipo_actualizacion == 'preview'){
				UIkit.notification({message:'<i class="fa fa-check"></i> El video resumen del curso ha sido subido con éxito.', status: 'success'});
				...
				UIkit.notification({message:'<i class="fa fa-check"></i>La portada del curso ha sido eliminada con éxito.', status: 'success'});
			}else if ($tipo_actualizacion == 'delete_preview'){
				UIkit.notification({message:'<i class="fa fa-check"></i>El video resumen del curso ha sido eliminado con éxito.', status: 'success'});
		...
		function updateFiles($curso){
			var url = {{ $www }};
            if (url == 1){
				var route = "https://www.transformatepro.com/ajax/multimedias-por-contenido/curso/"+$curso;
			}else{
				var route = "https://transformatepro.com/ajax/multimedias-por-contenido/curso/"+$curso;
			}
            //var route = "http://localhost:8000/ajax/multimedias-por-contenido/curso/"+$curso;
		...
		$(function(){
	        ...
	        $('.upload-input').on('change', function(e) {
	            ...
	            var url = {{ $www }};
            	if (url == 1){
	            	var route = "https://www.transformatepro.com/instructors/t-courses/update";
	            }else{
	            	var route = "https://transformatepro.com/instructors/t-courses/update";
	            }
            	//var route = "http://localhost:8000/instructors/t-courses/update";
            	var element = $(this).attr('id');
            	if (element == 'cover'){
            		var formData = new FormData(document.getElementById("form_cover"));
            	}else if (element == 'miniature_cover'){
            		var formData = new FormData(document.getElementById("form_miniature_cover"));
            	...
            	document.getElementById("miniature_cover").disabled = true;
	            ...
	            $.ajax({
	                ...
	                notificar(element);
	                updateFiles(document.getElementById("course_id").value);
	                document.getElementById("cover").disabled = false;
	                document.getElementById("miniature_cover").disabled = false;
	        ...
	        $('.delete-input').on('click', function(e) {
            	...
            	}else if (element == 'delete_miniature_cover'){
	            	document.getElementById("file_type").value = 'miniature_cover';
		...
		<div class="uk-card-default">
		    ...
		    <div class="uk-card-body">
				...
				<ul class="uk-switcher uk-margin uk-padding-small uk-container-small uk-margin-auto uk-margin-top">
					<li class="uk-active">
						<form class="uk-form-horizontal uk-margin-large" action="{{ route('instructors.courses.update') }}" method="POST" enctype="multipart/form-data" id="form_content">
							@csrf
							<input type="hidden" name="course_id" id="course_id" value="{{ $curso->id }}">
					    	<!--Datos Básicos -->
					    	<br>
					    	<div class="uk-grid">
					    		<div class="uk-width-1-1">
					    			<label class="uk-form-label" for="title"><b>Título:</b></label>
						            <input class="uk-input" id="title" name="title" type="text" placeholder="Título del Curso" value="{{ $curso->title }}"> 
					   			</div> 
							    ...
							    <div class="uk-width-1-2">
							        ...
							        <select class="uk-select" id="subcategory_id" name="subcategory_id">
							            ...
							            @foreach ($subcategorias as $subcategoria)
							                <option value="{{ $subcategoria->id }}" @if ($curso->subcategory_id == $subcategoria->id) selected @endif>
								                    	{{ $subcategoria->title }}
							                </option>
								        @endforeach
								    </select>
							    ...
							    <div class="uk-width-1-2">
						            ...
						            <select class="uk-select" name="mentor_route_time" id="mentor_route_time">
						                ...
						                <option value="0-3 Horas" @if ($curso->mentor_route_time == '0-3 Horas') selected @endif>Dedicaré un poco de mi tiempo en la Ruta del Mentor (0-3 horas)</option>
							            <option value="3-6 Horas" @if ($curso->mentor_route_time == '3-6 Horas') selected @endif>Trabajaré en mis tiempos libres en la Ruta del Mentor (3-6 Horas)</option>
							            <option value="+6 Horas" @if ($curso->mentor_route_time == '+6 Horas') selected @endif>Estoy enfocado en sacar lo más pronto posible la Ruta del Mentor (Más de 6 horas)</option>
							            <option value="+8 Horas" @if ($curso->mentor_route_time == '+8 Horas') selected @endif>Cumpliré mi objetivo en la Ruta del Mentor en el menor tiempo posible (Más de 8 horas)</option>
							        </select>
						    	</div> 

						    	<div class="uk-width-1-2">
						        	<label class="uk-form-label" for="price"><b>Precio:</b></label>
							        <input class="uk-input" id="price" name="price" type="number" value="{{ $curso->price }}"> 
								</div>
				    ...
				    <li>
				    	...
				    	<div class="uk-grid">
							<div class="uk-width-1-2">
							    <label class="uk-form-label" for="cover"><b>Portada del Curso:</b></label>
							    <form id="form_cover">
							...
							<div class="uk-width-1-2">
							    <label class="uk-form-label" for="miniature_cover"><b>Portada en Miniatura:</b></label>
							    <form id="form_miniature_cover">
							    	<input type="hidden" name="course_id" value="{{ $curso->id }}">
							        <input class="uk-input upload-input" type="file" name="miniature_cover" id="miniature_cover">
							    </form>
							</div>
						...
						<caption><h6>Recursos Multimedias del Curso</h6></caption>
						<div class="uk-overflow-auto">
							<table class="uk-table uk-table-middle uk-table-responsive">
								...
								<tbody id="content">
									...
									@endif
									@if (!is_null($curso->miniature_cover))
										<tr>
											<td class="uk-text-center uk-table-shrink">{{ $curso->miniature_cover_name }}</td>
											<td class="uk-text-center uk-table-shrink">
												<i class="fa fa-file-image img-icon"></i>
											</td>
											<td class="uk-text-center uk-table-shrink">Portada en Miniatura</td>
											<td class="uk-text-right">
												<div uk-lightbox>
													<a href="{{ asset('uploads/images/courses/thumbnails'.$curso->miniature_cover) }}" class="uk-icon-button uk-button-primary" uk-icon="icon: search;"></a>
												</div>
											</td>
											<td class="uk-text-left">
												<a href="#" class="uk-icon-button uk-button-danger delete-input" uk-icon="icon: trash;" id="delete_miniature_cover"></a>
											</td>
										</tr>
									@endif
