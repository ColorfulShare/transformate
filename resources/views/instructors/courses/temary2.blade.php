...
@push('scripts')
	...
	<script>
		function notificar($tipo_actualizacion){
			...
			}else if ($tipo_actualizacion == 'video_leccion_error'){
                UIkit.notification({message:'<i class="fa fa-times"></i> El archivo no se ha podido cargar. Intente nuevamente.', status: 'danger'});
			...
			}else if ($tipo_actualizacion == 'error_nuevo_recurso'){
                UIkit.notification({message:'<i class="fa fa-check"></i> El recurso no se ha podido cargar. Intente nuevamente.', status: 'danger'});
		...
		$(function(){
	        ...
	        $('.btn-submit-modulo').on('click', function(e) {
            	...
            	var url = {{ $www }};
            	if (url == 1){
            		var route = "https://www.transformatepro.com/instructors/t-courses/update-temary";
            	}else{
            		var route = "https://transformatepro.com/instructors/t-courses/update-temary";
            	}
            	
            	//var route = "http://localhost:8000/instructors/t-courses/update-temary";
	        ...
	        $('.video-file-submit').on('change', function(e) {
                ...
                var curso = document.getElementById("course_id").value;

                $(".video-file-submit").each(function(index) {
                    document.getElementById($(this).attr('id')).disabled = true;
                });
                $(".resource-file-submit").each(function(index) {
                    document.getElementById($(this).attr('id')).disabled = true;
                });
                ...
                if (upFile) {
                    ...
                    var extension = ext[1];
                    var path =  'courses/'+curso+'/'+leccion_id[1]+'.'+extension;
	        ...
	        $('.resource-file-submit').on('change', function(e) {
                ...
                if (upFile) {
                    ...
                    var ext = upFile.name.split(".");
                    var extension = ext[1];
                    var path =  'courses/lessons/resources/'+id+'.'+extension;
                    var uploadParams = {Key: path, Body: upFile};
	        ...
	        $('.delete-input').on('click', function(e) {
            	...
            	if (url == 1){
            	    if (id[1] == 'lesson'){
            	        var route = "https://www.transformatepro.com/instructors/lessons/delete";
            	    }else{
            	        var route = "https://www.transformatepro.com/instructors/lessons/delete-resource";
            	    }
            	}else{
            	    if (id[1] == 'lesson'){
            		    var route = "https://transformatepro.com/instructors/lessons/delete";
            	    }else{
											...
											<table class="uk-table uk-table-middle uk-table-responsive">
												<thead style="background-color: #D8D7D7;">
													<tr>
														...
														<th class="uk-text-center uk-table-shrink">Acción</th>
													</tr>
												</thead>
