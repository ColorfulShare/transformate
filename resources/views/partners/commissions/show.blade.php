	        ...
	        <div class="uk-card-body" style="margin-left: 60px;">
	        	<div class="uk-grid-divider uk-child-width-expand@s" uk-grid>
				    <div>
			            ...
			            Tipo: <b>@if (!is_null($comision->purchase_detail_id)) Compra Interna @else Compra T-Events @endif</b><br>
			            Fecha: <b>{{ date('d-m-Y H:i A', strtotime("$comision->created_at -5 Hours")) }}</b><br>
			            @if ($comision->status == 0) 
			            	Estado: <span class="uk-label uk-label-danger">Sin Cobrar</span> <br>
			            @elseif ($comision->status == 1)
			            	Estado: <span class="uk-label uk-label-warning">En Proceso</span> <br>
			            @else 
			            	Estado: <span class="uk-label uk-label-success">Cobrada</span> <br>
			            @endif 
				    </div>
