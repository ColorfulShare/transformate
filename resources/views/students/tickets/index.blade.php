...
@section('content')
	<div class="uk-container" style="height: 100%; padding-top: 30px;">
		@if ($errors->any())
        ...
        <div class="uk-flex-inline uk-flex-middle uk-margin-small-bottom"> 
            ...
            <h4 class="uk-margin-remove"> Mis Tickets </h4>                   
        </div>
	                ...
	                <table class="uk-table uk-table-hover uk-table-middle uk-table-divider">
	                    <thead>
	                        <tr>
		                        ...
		                        <th class="uk-table-expand uk-text-center">Título</th> 
	                    	...
	                    	@if ($totalTickets > 0)
		                        @foreach ($tickets as $ticket)
		                            <tr>
		                                ...
		                                <td class="uk-width-small uk-text-center">
		                                	@if ($ticket->status == 3)
		                                		{{ date('d-m-Y', strtotime($ticket->resolved_at)) }}
		                                	@else
		                                		-
		                                	@endif
		                                </td>
		                                <td class="uk-width-small uk-text-center">
		                                	<a class="uk-icon-button uk-button-success" uk-icon="icon: search;" uk-tooltip="Ver Más" href="{{ route('students.tickets.show', $ticket->id) }}"></a>
		                                	@if ($ticket->status != 3)
		                                		<a class="uk-icon-button uk-button-danger" uk-icon="icon: ban;" uk-tooltip="Cerrar Ticket" href="{{ route('students.tickets.close', $ticket->id) }}"></a> 
		                                	@endif
		                                </td>
		                            </tr>    
		                        @endforeach
    ...
    <div id="create-modal" class="uk-modal-container" uk-modal> 
        <div class="uk-modal-dialog"> 
            ...
            <form action="{{ route('students.tickets.store') }}" method="POST">  
                @csrf
                <div class="uk-modal-body"> 
                    <div class="uk-grid">
                        ...
                        <div class="uk-width-1-1">
                            Descripción (*):
							<textarea class="ckeditor" name="description" rows="5">
							</textarea>
							<script>
								ClassicEditor
