...
@section('content')
    ...
    <div class="uk-width-3-4@m uk-margin-auto">
        <header class="tm-course-content-header uk-background-grey"> 
            <a href="{{ route('students.certifications.resume', [$certificacion->slug, $certificacion->id]) }}" class="back-to-dhashboard uk-margin-large-left" uk-tooltip="title: Volver al Resumen de la T-Mentoring ; delay: 200 ; pos: bottom-left ;animation:uk-animation-scale-up ; offset:20"> Volver a la T-Mentoring</a> 
        </header>
        ...
        <div class="uk-padding animation: uk-animation-slide-left-medium, uk-animation-slide-right-medium"> 
            <div class="video-responsive">
                @if (!is_null($leccionEscogida->video))    
                    <video src="{{ $leccionEscogida->video }}" controls controlslist="nodownload" style="width: auto; height: auto;" id="current-video"></video>
                @else
    ...
    <div class="uk-width-1-4@m uk-offcanvas tm-filters uk-background-default tm-side-course uk-animation-slide-right-medium" id="filters" uk-offcanvas="overlay: true; container: false; flip: true">
        <div class="uk-offcanvas-bar uk-padding-remove uk-preserve-color">
            <div class="course-content">
                Contenido de la T-Mentoring
            </div>
            <div class="modules-content">
                <ul uk-accordion="multiple: true" class="lessons-accordion">
                    @foreach ($certificacion->modules as $modulo)
                        <li class="title-li uk-padding-small" @if ($modulo->priority_order == 1) style="margin-top: 20px;" @endif>
                            <a class="uk-accordion-title accordion-lesson-title" href="#" style="color: #505763;">
                                Módulo {{ $modulo->priority_order }}: {{ $modulo->title }}<br>
                                <span class="font-text-xs completed-lessons">{{ $modulo->completed_lessons }} / 6 <i class="fa fa-check"></i></span> {{--  |<span class="font-text-xs duration-module">7 min</span>--}}
                            </a> 
                                ...
                                <ul class="lessons-list">
                                    @foreach ($modulo->lessons as $leccion)
                                        <li aria-current="true" class="lesson video @if ($leccion->finished == 0) current @endif" data-lessonId="{{ $leccion->id }}">
                                            <a href="{{ route('students.certifications.lessons', [$certificacion->slug, $certificacion->id, $leccion->id]) }}" id="link-lesson-{{ $leccion->id }}"></a>
                                            <div class="item-link item-link-default demo-check" tabindex="0" role="link">
                                                ...
                                                <div class="lesson-info">
                                                    <div class="lesson-title">
                                                        <span width="0">
                                                            <span>
                                                                <span>{{ $leccion->priority_order }}. {{ $leccion->title }}</span>
                                                            </span>
                                                        </span>
                                                    </div>
