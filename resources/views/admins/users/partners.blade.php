...
@section('content')
    <div class="admin-content-inner"> 
        <div class="uk-flex-inline uk-flex-middle uk-margin-small-bottom"> 
            ...
            <h4 class="uk-margin-remove"> T-Partners </h4> 
            @if (Auth::user()->profile->users == 2)
                <button class="uk-button uk-button-success uk-margin-medium-left admin-btn" href="#create-modal" uk-tooltip="title: Crear Alumno; delay: 500 ; pos: top ;animation: uk-animation-scale-up" uk-toggle> 
                    <i class="fas fa-user-plus"></i> Nuevo T-Partner
                </button> 
            @endif                        
        </div>  
                ...
                <table id="users-table"> 
                    ...
                    <tbody> 
                        @foreach ($partners as $partner)
                            <tr>                                  
                                <td class="uk-text-center">{{ $partner->names }} {{ $partner->last_names }}</td> 
                                <td class="uk-text-center">{{ $partner->email }}</td> 
                                <td class="uk-text-center">{{ $partner->country }}</td> 
                                <td class="uk-text-center">{{ $partner->partner_code }}</td> 
                                <td class="uk-flex-inline uk-text-center"> 
                                    <a href="{{ route('admins.users.show', $partner->id) }}" class="uk-icon-button uk-button-success btn-icon" uk-icon="icon: search;" uk-tooltip="Ver - Editar"></a> 
                                    @if (Auth::user()->profile->users == 2)
        ...
        <div class="uk-modal-dialog"> 
            ...
            <form action="{{ route('admins.users.store') }}" method="POST">  
                ...
                <div class="uk-modal-footer uk-text-right"> 
                    ...
                    <button class="uk-button uk-button-primary" type="submit" id="btn-crear">Crear T-Partner</button>
                </div>     
            </form>                        
        </div>                         
    </div>
