    ...
    <script>
        $(function(){
            ...
            $('.form-cover').on('submit',function(e){
                e.preventDefault();
                $.ajax({
                    url:"{{ route('admin.courses.load-cover-image') }}",
                    type:'POST',
	...
	<div class="admin-content-inner"> 
        <nav class="uk-navbar-container uk-margin-medium-bottom" uk-navbar style="padding-top: 20px; padding-bottom: 40px;">
            ...
            <div class="uk-navbar-right">
                <ul class="uk-navbar-nav">
                    <li class="uk-active" style="border-right: 1px solid #000;"><a href="{{ route('admins.courses.home-cover') }}">Portada del Home</a></li>
                    ...
                    <tbody> 
                        @if (!is_null($portadaActual))
                            <tr style="background-color: #D6F9B6;" uk-tooltip="Portada Actual">                          
                                <td class="uk-text-center">
                                    <img class="uk-preserve-width uk-border-circle user-profile-small" src="{{ asset('uploads/images/courses/'.$portadaActual->cover) }}" width="50">
                                </td>                                 
                        ...
                        @foreach ($cursosDisponibles as $curso)
                            <tr>                          
                                ...
                                <td class="uk-text-center">
                                    <label><input class="uk-radio select-cover" type="radio" name="select-cover" data-id="{{ $curso->id }}"> </label>
                                </td>                                     
                            </tr>  
                        @endforeach                      
                    </tbody>                         
