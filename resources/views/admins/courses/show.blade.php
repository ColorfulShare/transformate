	                    ...
	                    <div class="uk-width-1-2">
	                    	...
	                    	<select class="uk-select" name="category_id" id="category_id" onchange="cargarSubcategorias();" required>
	                    		@foreach ($categorias as $categoria)
	                                <option value="{{ $categoria->id }}" @if ($categoria->id == $curso->category_id) selected @endif>{{ $categoria->title }}</option>
	                            @endforeach
	                        </select>
	                    </div>
	                     <div class="uk-width-1-2">
	                    	...
	                    	<select class="uk-select" name="subcategory_id" id="subcategory_id" required>
	                    		@foreach ($subcategorias as $subcategoria)
	                                <option value="{{ $subcategoria->id }}" @if ($subcategoria->id == $curso->subcategory_id) selected @endif>{{ $subcategoria->title }}</option>
	                            @endforeach
	                        </select>
	                    </div>
	                    <div class="uk-width-1-3">
	                    	Instructor: 
	                    	<input class="uk-input" type="text" value="{{ $curso->user->names }} {{ $curso->user->last_names }}" disabled>
	                    </div>
	                    <div class="uk-width-1-3">
	                    	Ruta del Instructor: 
	                    	<select class="uk-select" name="mentor_route_time" required>
							    <option value="0-3 Horas" @if ($curso->mentor_route_time == '0-3 Horas') selected @endif>0-3 horas</option>
							    <option value="3-6 Horas" @if ($curso->mentor_route_time == '3-6 Horas') selected @endif>3-6 Horas</option>
							    <option value="+6 Horas" @if ($curso->mentor_route_time == '+6 Horas') selected @endif>+6 Horas</option>
							    <option value="+8 Horas" @if ($curso->mentor_route_time == '+8 Horas') selected @endif>+8 Horas</option>
							</select>
	                    </div> 
	                    <div class="uk-width-1-3">
	                    	Precio (COP):
	                    	<input class="uk-input" name="price" type="number" value="{{ $curso->price }}" required> 
	                    </div>     
	                    <div class="uk-width-1-1">
	                    	Reseña:
	                    	<textarea class="ckeditor" name="review" rows="5" placeholder="Reseña del Curso" required>{{ $curso->review }}</textarea>
	                    </div>
	                    <div class="uk-width-1-1">
	                    	Descripción:
	                    	<textarea class="ckeditor" name="description" rows="5" placeholder="Descripción del Curso" required>{{ $curso->description}}</textarea>
	                    </div>
	                    <div class="uk-width-1-1">
	                    	Objetivos:
	                    	<textarea class="ckeditor" name="objectives" rows="5" placeholder="Objetivos del Curso" required>{{ $curso->objectives }}</textarea>
	                    </div>
	                    <div class="uk-width-1-1">
	                    	Requisitos Previos:
	                    	<textarea class="ckeditor" name="requirements" rows="5" placeholder="Requisitos Previos del Curso" required>{{ $curso->requirements }}</textarea>
	                    </div>
	                    <div class="uk-width-1-1">
	                    	Destino:
	                    	<textarea class="ckeditor" name="destination" rows="5" placeholder="A quién va diridigo el curso" required>{{ $curso->destination }}</textarea>
	                    </div>
	                    <div class="uk-width-1-1">
	                    	Material Incluido:
	                    	<textarea class="ckeditor" name="material_content" rows="5" placeholder="Material incluido en el curso" required>{{ $curso->material_content }}</textarea>
	                    </div>
	                    <div class="uk-width-1-1">
							Etiquetas:
						</div>
